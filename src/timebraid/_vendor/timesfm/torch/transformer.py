...
class RotaryPositionalEmbedding(nn.Module):
  ...
  def forward(
    ...
    # Keep RoPE numerics stable by building the sinusoid in fp32, but cast the
    # final trigonometric factors back to the input dtype before mixing them
    # with the activations. Without this, bf16 TimesFM activations get promoted
    # to fp32 in the rotary path while V stays bf16, and fused SDPA then fails
    # because Q/K/V no longer share a dtype.
    rope_dtype = torch.float32
    fraction = (
      2
      * torch.arange(0, half_embedding_dim, device=inputs.device, dtype=rope_dtype)
      / self.embedding_dims
    )
    timescale = (
      self.min_timescale * (self.max_timescale / self.min_timescale) ** fraction
    ).to(device=inputs.device, dtype=rope_dtype)
    if position is None:
      seq_length = inputs.shape[1]
      position = torch.arange(seq_length, dtype=rope_dtype, device=inputs.device)[
        None, :
      ]
    else:
      position = position.to(device=inputs.device, dtype=rope_dtype)
    ...
    sin = torch.sin(sinusoid_inp).to(dtype=inputs.dtype)
    cos = torch.cos(sinusoid_inp).to(dtype=inputs.dtype)
